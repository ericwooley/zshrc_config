...
zshsetup() {
  ...
  local -a remote_steps=(
    ...
    'cd "$repo_dir"'
    'sh ./install.sh'
    'printf "zshsetup: install 10-minute zshupdate cron on this remote? [y/N] "; read cron_answer || cron_answer=""; case "$cron_answer" in y|Y|yes|YES) ZSHRC_CONFIG_DIR="$repo_dir" zsh -lc "source \"$repo_dir/functions/zsh_install_hourly_update_cron.zsh\"; zsh_install_hourly_update_cron" ;; *) echo "zshsetup: skipped 10-minute zshupdate cron" ;; esac'
