...
coverage-valid implementation review as reviewed context and do not re-review
the feature, correction delta, or unrelated checklist items. A mixed pass with
...
`Review focus: outstanding evidence only`, use only the checklist items needed
to adjudicate the supplied evidence or handoff requests and do not repeat the
preceding code review. Otherwise, in `final follow-up`, evaluate the original
request and plan only as context for the supplied correction or
intentional-drift purpose and regressions introduced by that delta; treat
earlier coverage-valid completion evidence as reviewed. Do not re-review the
feature as a whole.
...
- `Integration assessment`: for a checkpoint, whether the reviewed delta fits
  ...
  repeating the preceding implementation review;
